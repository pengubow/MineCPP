...
void NameLevelScreen::init() {
    ...
    buttons.push_back(make_shared<Button>(0, width / 2 - 100, height / 4 + 120, "Save"));
    buttons.push_back(make_shared<Button>(1, width / 2 - 100, height / 4 + 144, "Cancel"));
...
void NameLevelScreen::render(int32_t var1, int32_t var2) {
    ...
    drawString(font, name + (counter / 6 % 2 == 0 ? "_" : ""), var3 + 4, var4 + 6, 14737632);
